...
TEST_CASE("Lexer can tokenize keywords", "[lexer][phase1]") {
    LexerTestHelper helper;
    auto tokens = helper.tokenize("if else while for func return true false null");
...
TEST_CASE("Lexer handles invalid characters with error recovery", "[lexer][phase1]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can parse integers with underscore separators", "[lexer][phase2]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can parse modern type suffixes", "[lexer][phase2]") {
    LexerTestHelper helper;
    auto tokens = helper.tokenize("42i8 255u8 1000i16 65535u16 42i32 4000000000u32 42i64 18446744073709551615u64");
...
TEST_CASE("Lexer can tokenize complex expressions with Phase 2 integers", "[lexer][phase2]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can parse basic decimal floats", "[lexer][phase3]") {
    LexerTestHelper helper;
    auto tokens = helper.tokenize("3.14 .5 5. 0.0");
...
TEST_CASE("Lexer can parse scientific notation", "[lexer][phase3]") {
    LexerTestHelper helper;
    auto tokens = helper.tokenize("1e10 1.5e-3 2E+5 .5e2");
...
TEST_CASE("Lexer can parse floats with underscore separators", "[lexer][phase3]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer handles malformed floating-point literals", "[lexer][phase3]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer distinguishes integers from floats correctly", "[lexer][phase3]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can tokenize complex expressions with Phase 3 floats", "[lexer][phase3]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can parse basic string literals", "[lexer][phase4]") {
    ...
    CHECK(helper.getTokenText(tokens[0]) == "\"Hello, World!\"");
    
    CHECK(tokens[1].kind == TokenKind::StringLiteral);
    CHECK(helper.getTokenText(tokens[1]) == "\"\"");
    
    CHECK(tokens[2].kind == TokenKind::StringLiteral);
    CHECK(helper.getTokenText(tokens[2]) == "\"test\"");
    
    CHECK(tokens[3].kind == TokenKind::EoF);
}

TEST_CASE("Lexer can parse strings with standard escape sequences", "[lexer][phase4]") {
    LexerTestHelper helper;
    auto tokens = helper.tokenize(R"("Line 1\nLine 2\tTabbed\r\nWindows\\\"Quote")");
    
    REQUIRE(tokens.size() == 2); // 1 string + EOF
    CHECK(tokens[0].kind == TokenKind::StringLiteral);
    // The actual escape processing would be verified through the token's internal representation
    CHECK(tokens[1].kind == TokenKind::EoF);
}

TEST_CASE("Lexer can parse strings with hex escape sequences", "[lexer][phase4]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can parse strings with Unicode escapes", "[lexer][phase4]") {
    LexerTestHelper helper;
    auto tokens = helper.tokenize(R"("Unicode: \u0041\u{1F680}\U00000042")");
...
TEST_CASE("Lexer can parse character literals with escape sequences", "[lexer][phase4]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can parse character literals with hex escapes", "[lexer][phase4]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer can tokenize mixed expressions with strings and chars", "[lexer][phase4]") {
    LexerTestHelper helper;
    ...
    for (const auto& token : tokens) {
        if (token.kind == TokenKind::StringLiteral) foundString = true;
        if (token.kind == TokenKind::CharLiteral) foundChar = true;
    }
...
TEST_CASE("Lexer handles multiline raw strings", "[lexer][phase4]") {
    ...
    std::string multilineRawSource = "r\"Line 1\nLine 2\n\\n literal backslash-n\"";
...
TEST_CASE("Lexer handles string literals with escape sequences and multiline", "[lexer][phase4]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer distinguishes raw strings from regular identifiers", "[lexer][phase4]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer handles mixed string types in expressions", "[lexer][phase4]") {
    LexerTestHelper helper;
    
    // Test mixing regular strings, raw strings, and other tokens
    auto tokens = helper.tokenize("func test() { \"regular\" + r\"raw\\nstring\" == 'c' }");
...
TEST_CASE("Lexer handles line comments", "[lexer][phase5]") {
    ...
    auto tokens2 = helper.tokenize("func test() { // Comment with symbols!@#$%^&*()");
...
TEST_CASE("Lexer handles nested block comments", "[lexer][phase5]") {
    ...
    auto tokens2 = helper.tokenize("/* level 1 /* level 2 /* level 3 */ level 2 */ level 1 */ x;");
...
TEST_CASE("Lexer handles mixed comments", "[lexer][phase5]") {
    ...
    for (const auto& token : tokens) {
        if (token.kind == TokenKind::Ident) {
            ...
            if (text == "x") foundX = true;
            if (text == "y") foundY = true;
        }
...
TEST_CASE("Lexer handles bitwise compound assignment operators", "[lexer][phase1.5]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer handles increment and decrement operators", "[lexer][phase1.5]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer handles advanced punctuation and macros", "[lexer][phase1.5]") {
    LexerTestHelper helper;
...
TEST_CASE("Lexer distinguishes between similar operators", "[lexer][phase1.5]") {
    LexerTestHelper helper;
    
    // Test that we distinguish + vs ++ vs +=
    auto tokens1 = helper.tokenize("+ ++ +=");
    std::vector<TokenKind> expected1 = {
        TokenKind::Plus, TokenKind::PlusPlus, TokenKind::PlusEqual, TokenKind::EoF
    };
    ...
    std::vector<TokenKind> expected2 = {
        TokenKind::Less, TokenKind::Shl, TokenKind::LessEqual, TokenKind::ShlEqual, TokenKind::EoF
    };
    ...
    std::vector<TokenKind> expected3 = {
        TokenKind::BAnd, TokenKind::LAnd, TokenKind::BAndEqual, TokenKind::BAndDot, TokenKind::EoF
    };
...
TEST_CASE("Lexer handles complex operator expressions", "[lexer][phase1.5]") {
    ...
    // Should have the correct number of tokens (not going to verify every one, just basics)
    ...
    for (const auto& token : tokens) {
        switch (token.kind) {
            case TokenKind::PlusEqual: foundPlusEqual = true; break;
            case TokenKind::PlusPlus: foundPlusPlus = true; break;
            case TokenKind::Shl: foundShl = true; break;
            case TokenKind::BAndEqual: foundBAndEqual = true; break;
            case TokenKind::ThinArrow: foundThinArrow = true; break;
            case TokenKind::FatArrow: foundFatArrow = true; break;
            case TokenKind::LOr: foundLOr = true; break;
            case TokenKind::LParen: foundLParen = true; break;
            case TokenKind::LBracket: foundLBracket = true; break;
            default: break;
        }
    }
